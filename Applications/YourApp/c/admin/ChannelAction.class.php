...
class ChannelAction extends IndexAction{
    ...
    public function index(){
        ...
        $this->smarty->display('Channel/content.html');
    }
    ...
    public function add(){
        ...
        $this->fieldArr = category::getField($mid);
        if(isset($_POST['addInfo'])){
            ...
            rewrite::succ('增加信息成功','?m=Channel&a=index&classid='.$this->classid);
        }
        ...
        $this->smarty->display('Channel/addcontent.html');
    }
    ...
    public function update(){
        ...
        if(isset($_POST['updateInfo'])){
            $checked_data = $this->check();
            $this->contentModel->updateInfo($checked_data);
        ...
        $this->smarty->assign('formdir',ROOT_PATH.'data/form/'.$mid.'.php');
        $this->smarty->display('Channel/updatecontent.html');
    }

    //删除信息控制
    public function delete(){
        $delinfo = $this->contentModel->get($this->id, $this->classid);
        $this->contentModel->delete($this->id, $delinfo);
    ...
    public function setCallbackUrl(){
        global $config;
        if($this->contentModel->setCallbackUrl($config['liveCallbackUrl'])){
            addlog('设置录制回调地址'.$config['liveCallbackUrl']);
            rewrite::succ('设置成功');
        }else{
            rewrite::succ('设置失败，未知错误！');
        }
...
*/
    ...
    private function check(){
        $data = p(1,1);
        if(empty($data['name'])) rewrite::js_back('【频道名称】不能为空');
		...
		if(isset($_POST['updateInfo'])){
		   ...
		   $newData[1]['cid'] = $data['cid'];
           $newData[1]['type'] = $data['type'];
           $newData[1]['price'] = $data['price'];
           $newData[1]['thumb'] = $data['thumb'];
           $newData[1]['content'] = $data['content'];
		}
		if(isset($_POST['addInfo'])){
		   $newData[1]['classid'] = $data['classid'];
		}
        $newData[1]['teacher'] = $data['teacher'];
        $newData[1]['bay'] = $data['bay'];
        $newData[1]['name'] = $data['name'];
        ...
        $newData[1]['overtime'] = strtotime($data['overtime']);
        //print_r($newData[1]);
		return $newData;
    }
