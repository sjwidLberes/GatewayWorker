...
class page{
    ...
    private function listpage_2(){
        ...
        for($i=1;$i<=$h;$i++) {
            $page = $this->page+$i;
            if ($page > $this->pagenum) break;
            $pageList .= '<a href="'.$this->url.$page.'.html">'.$page.'</a>';
        }
        if($this->page > 1){
            $prevNum = ($this->page-1) < 1 ? 1 : $this->page - 1;
            $prev = '<a href="'.$this->url.$prevNum.'.html">上一页</a>';
        }
        if($this->page+1 <= $this->pagenum){
            $nextNum = ($this->page + 1) > $this->pagenum ? $this->pagenum : $this->page + 1;
            $next = '<a href="'.$this->url.$nextNum.'.html">下一页</a>';
        }
        if($this->page > 1){
            $home = '<a href="'.$this->url.'1.html">首页</a>';
        }
        if($this->page+1 <= $this->pagenum){
            $last = '<a href="'.$this->url.$this->pagenum.'.html">尾页</a>';
        }
        if($this->pagenum > 1){
